...
async def search_refuting_evidence(
    claim: Annotated[
        str,
        Field(description="The clinical claim to find contradicting evidence for"),
    ],
    patientId: Annotated[
        str | None,
    ...
    all_resources = conditions + observations + medications + procedures + documents + consents
    resource_summaries = [summarize_resource(r) for r in all_resources]

    all_evidence = []
    for assertion in assertions:
        try:
            evidence_items = await evaluate_evidence(assertion, resource_summaries, "refuting")
    ...
    return json.dumps({
        ...
        "refuting_evidence": all_evidence,
